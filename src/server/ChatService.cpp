...
void ChatService::login(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPwd() == pwd)
    {
        ...
        else
        {
            ...
            // 查询该用户是否有离线消息
            vector<string> vec = _offlineMsgModel.quiry(id);
            if (!vec.empty())
            {
                response["offlinemsg"] = vec;
                // 读取该用户的离线消息后，把该用户的所有离线消息删掉
                _offlineMsgModel.remove(id);
            }
            // 查询该用户的所有好友信息并返回
            vector<User> userVec = _friendModel.quiry(id);
            if (!userVec.empty())
            {
                vector<string> vec2;
            ...
            // 查询用户的群组信息
            vector<Group> groupusenVec = _groupModel.queryGroups(id);
            if (!groupusenVec.empty())
            {
                // group:[{groupid:[xxx, xxx, xxx, xxx]}]
                vector<string> groupV;
                for (Group &group : groupusenVec)
                {
                    json grpjson;
